...
setup("authenticate", async ({ page }) => {
  ...
  await expect(page.getByText("🌸 SakuMari 🌸")).toBeVisible({ timeout: 15000 });
  ...
  const credentialsSubmit = page.locator('form').filter({ has: page.locator('input[name="email"]') }).locator('button[type="submit"]');

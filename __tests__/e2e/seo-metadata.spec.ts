...
test.describe("SEO and Metadata", () => {
  ...
  test("should have unique page titles", async ({ page }) => {
    // Test accessible pages (protected pages redirect to home for unauthenticated users)
    await page.goto("/");
    const homeTitle = await page.title();
    expect(homeTitle).toContain("SakuMari");
    
    // Protected pages redirect to home, so they'll have the same title when unauthenticated
  ...
  test("should have meaningful content", async ({ page }) => {
    ...
    await expect(page.locator("text=Welcome to SakuMari!").first()).toBeVisible();

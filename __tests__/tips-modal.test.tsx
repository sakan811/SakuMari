...
describe("TipsModal", () => {
  ...
  // Tests from tips-modal-uncovered.test.tsx
  test("prevents submission when input is empty", () => {
    render(<TipsModal isOpen={true} onClose={mockOnClose} />);
  ...
  test("prevents submission when already loading", async () => {
    ...
    // Now try to submit again while still loading
  ...
  test("handles API error with custom error message", async () => {
    const errorResponse = {
      ok: false,
      status: 400,
  ...
  test("handles API error with default error message when no error is provided", async () => {
    const errorResponse = {
      ok: false,
      status: 500,
